...
local function GetDate(elems)
  AddElement(elems, HEADER_DATE,
    string.format(" %s(%s)",
      wezterm.strftime("%-m/%-d"),
      dayOfWeekInJapanese(tonumber(wezterm.strftime("%u")))
      )
    )
end
...
local function GetBattery(elems, window)
  for _, b in ipairs(wezterm.battery_info()) do
    local battery = HEADER_BATTERY
    if b.state == 'Charging' then
      battery = HEADER_BATTERY_CHARGE
    end
    AddElement(elems, battery, string.format('%.0f%%', b.state_of_charge * 100))
  end
end
